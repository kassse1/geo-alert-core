# 🚨 Geo Alert Core

**Geo Alert Core** — backend-сервис на Go, реализующий ядро системы геооповещений.  
Сервис принимает координаты пользователей, возвращает список ближайших опасных зон и при наличии угроз **асинхронно отправляет вебхуки** во внешний портал (например, новостной сервис на Django).

Проект построен с использованием **Clean Architecture** и ориентирован на production-подход.

---

## 📌 Общая идея системы

Система состоит из трёх логических компонентов:

- 📱 **Мобильное приложение**  
  Отправляет координаты пользователя

- 🧠 **Geo Alert Core (данный сервис)**  
  Определяет наличие опасных зон поблизости и управляет инцидентами

- 🌐 **Внешний портал (Django)**  
  Получает уведомления через вебхуки

---

## 🧠 Архитектура

Проект реализован по принципам **Clean Architecture**:

HTTP Handler

↓

Service (Business Logic)

↓

Repository (PostgreSQL)

### Слои системы

- **Handler** — HTTP-уровень, валидация входящих запросов  
- **Service** — бизнес-логика (гео-проверка, статистика, вебхуки)  
- **Repository** — работа с PostgreSQL  
- **Middleware** — API-key авторизация  
- **Config** — конфигурация через environment variables  
- **Transport** — маршрутизация и инициализация HTTP-сервера  

---

## 🚀 Функциональность

### 1️⃣ Управление инцидентами  
CRUD-операции, защищённые API-ключом.

| Метод | Endpoint | Описание |
|------|---------|----------|
| POST | `/api/v1/incidents` | Создание инцидента |
| GET | `/api/v1/incidents` | Получение списка (пагинация) |
| GET | `/api/v1/incidents/{id}` | Получение по ID |
| PUT | `/api/v1/incidents/{id}` | Обновление |
| DELETE | `/api/v1/incidents/{id}` | Деактивация (soft delete) |

🔐 Все эндпоинты требуют заголовок `X-API-Key`.


### 2️⃣ Проверка координат (публичный API)

**POST** `/api/v1/location/check`

Поведение:
- Клиент отправляет координаты
- Сервис синхронно возвращает список активных опасных зон поблизости
- Факт проверки сохраняется в БД
- При наличии угроз **асинхронно отправляется вебхук**

### 3️⃣ Статистика

**GET** `/api/v1/incidents/stats?minutes=N`

Возвращает количество **уникальных пользователей**, которые выполняли проверку координат за последние `N` минут.

Пример ответа:

{
  "user_count": 3
}

### 4️⃣ Health Check

GET /api/v1/system/health

Используется для мониторинга состояния сервиса.

---
 
🔔 Вебхуки

Отправляются асинхронно

Не блокируют основной HTTP-ответ

URL задаётся через WEBHOOK_URL

Для тестирования используется HTTP-сервер-заглушка на :9090

---

🛠 Используемые технологии

Go 1.24+

PostgreSQL 15

Docker / Docker Compose

REST API

Clean Architecture

Haversine formula (гео-расчёты)

---

⚙️ Конфигурация (.env)
APP_PORT=8080
POSTGRES_DSN=postgres://danger:danger@localhost:5433/danger?sslmode=disable
API_KEY=super-secret-key
STATS_TIME_WINDOW_MINUTES=5
WEBHOOK_URL=https://undeficient-itchingly-janel.ngrok-free.dev/webhook

---

🧪 Проверка вебхуков (End-to-End)

Запусти webhook-mock

Создай активный инцидент

Выполни location/check с координатами рядом с инцидентом

В консоли webhook-сервера появится входящий JSON

---

```json
▶️ Запуск проекта

1️⃣ Запуск PostgreSQL
docker compose up -d

2️⃣ Запуск webhook-заглушки
go run ./cmd/webhook-mock

3️⃣ Запуск ngrok
ngrok http 9090

4️⃣ Запуск основного API
go run ./cmd/api/main.go

🧪 Примеры запросов (PowerShell)
1️⃣ ЗАПУСК ИНФРАСТРУКТУРЫ (PostgreSQL)
docker compose up -d
Проверка : docker ps
Ты должен увидеть контейнер с PostgreSQL.

2️⃣ ЗАПУСК WEBHOOK-СЕРВЕРА (ЗАГЛУШКА)
go run ./cmd/webhook-mock
Ожидаемый вывод : Webhook mock listening on :9090

3️⃣ ЗАПУСК NGROK 
ngrok http 9090

4️⃣ ЗАПУСК ОСНОВНОГО API
go run ./cmd/api/main.go
Ожидаемый вывод : Server started on port 8080

5️⃣ HEALTH CHECK
Invoke-RestMethod http://localhost:8080/api/v1/system/health
Ожидаемый ответ : {"status":"ok"}

6️⃣ СОЗДАНИЕ ИНЦИДЕНТА (CRUD — CREATE)
Invoke-RestMethod `
  -Uri "http://localhost:8080/api/v1/incidents" `
  -Method Post `
  -Headers @{ "X-API-Key" = "super-secret-key" } `
  -ContentType "application/json" `
  -Body (@{
    title = "Fire near mall"
    lat = 43.23
    lon = 76.88
    radius_m = 500
  } | ConvertTo-Json)
Ожидаемо : JSON с id.

7️⃣ ПОЛУЧЕНИЕ СПИСКА ИНЦИДЕНТОВ (READ + PAGINATION)
Invoke-RestMethod `
  -Uri "http://localhost:8080/api/v1/incidents?page=1&limit=10" `
  -Headers @{ "X-API-Key" = "super-secret-key" }

8️⃣ ПОЛУЧЕНИЕ ИНЦИДЕНТА ПО ID
(подставь реальный ID)
Invoke-RestMethod `
  -Uri "http://localhost:8080/api/v1/incidents/1" `
  -Headers @{ "X-API-Key" = "super-secret-key" }

9️⃣ ОБНОВЛЕНИЕ ИНЦИДЕНТА (UPDATE)
Invoke-RestMethod `
  -Uri "http://localhost:8080/api/v1/incidents/1" `
  -Method Put `
  -Headers @{ "X-API-Key" = "super-secret-key" } `
  -ContentType "application/json" `
  -Body (@{
    title = "Updated fire"
    lat = 43.24
    lon = 76.89
    radius_m = 600
  } | ConvertTo-Json)
Ожидаемо: 204 No Content.

🔟 ПРОВЕРКА КООРДИНАТ (PUBLIC API)
Invoke-RestMethod `
  -Uri "http://localhost:8080/api/v1/location/check" `
  -Method Post `
  -ContentType "application/json" `
  -Body (@{
    user_id = "user_test"
    lat = 43.23
    lon = 76.88
  } | ConvertTo-Json)


Ожидаемо : JSON со списком инцидентов
webhook уходит асинхронно

1️⃣1️⃣ ПРОВЕРКА ВЕБХУКА (END-TO-END)
В webhook-mock ты должен увидеть:
WEBHOOK RECEIVED:
map[user_id:user_test incidents:[...] sent_at:...]
В ngrok : POST /webhook 200 OK

1️⃣2️⃣ СТАТИСТИКА
Invoke-RestMethod `
  -Uri "http://localhost:8080/api/v1/incidents/stats?minutes=5" `
  -Headers @{ "X-API-Key" = "super-secret-key" }
Ожидаемый ответ : {"user_count":1}

1️⃣3️⃣ ДЕАКТИВАЦИЯ ИНЦИДЕНТА (DELETE = SOFT DELETE)
Invoke-RestMethod `
  -Uri "http://localhost:8080/api/v1/incidents/1" `
  -Method Delete `
  -Headers @{ "X-API-Key" = "super-secret-key" }

1️⃣4️⃣ ПРОВЕРКА, ЧТО ИНЦИДЕНТ БОЛЬШЕ НЕ ВЫДАЁТСЯ
Invoke-RestMethod `
  -Uri "http://localhost:8080/api/v1/location/check" `
  -Method Post `
  -ContentType "application/json" `
  -Body (@{
    user_id = "user_test_2"
    lat = 43.23
    lon = 76.88
  } | ConvertTo-Json)
Ожидаемо : []
